...
eugene_okulik_path = os.path.join(homew_path, 'eugene_okulik', 'data', 'logs') # Формирование пути к каталогу с логами
# print(eugene_okulik_path)
...
def get_files(path, search_text=None):
    for file in os.listdir(path):
        if os.path.isfile(os.path.join(path, file)):
            yield file
for file in get_files(eugene_okulik_path, 'error'):
    search_text = "ERROR"
    with open(os.path.join(eugene_okulik_path, file), 'r', encoding='utf-8') as f:
        content = f.read()
        if search_text in content:
            ...
            for i, word in enumerate(words):
                if search_text in word.upper():  # Поиск без учета регистра
                    ...
                    final_path = os.path.join(eugene_okulik_path, file)  # Выводим результат
