...
from page.desk1_page import (FILTER_NEWEST, FILTER_NAME_AZ,
                              FILTER_PRICE_LOW, FILTER_PRICE_HIGH)
...
"""Тест перехода на другую стр с установкой чекбокса на steel - NO WORK!"""
...
"""Тест фильтров сортировки — 4 теста в одном"""
@pytest.mark.parametrize("filter_text, expected_url, filter_name", FILTER_PARAMS)
def test_apply_filter(page: Page, filter_text: str, expected_url: str, filter_name: str):
...
def test_search_locators_in_element(page: Page):
    shop_click = Desk1Page(page)
    shop_click.open()
