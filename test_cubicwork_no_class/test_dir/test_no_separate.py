...
@pytest.mark.smoke
def test_status_upload_advert_file(new_token):
...
def test_create_and_update_advert_camp(new_token):
    ...
    create_response = requests.post('https://aooh-test.cubicservice.ru/v1/campaigns/new?counterpartyId=7',
                                    json=create_body, headers=headers)

    assert create_response.status_code == 200, f"Ошибка: {create_response.status_code} - {create_response.text}"
    ...
    assert update_response.status_code == 200, f"Ошибка: {update_response.status_code} - {update_response.text}"
